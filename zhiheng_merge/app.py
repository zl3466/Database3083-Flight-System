...
def public_flightSearch():
    return flight_search('public_view_flights.html')
...
def public_flightSearchRT():
    return flight_search_rt('public_view_flights.html')
...
def customer_search_flight():
    return flight_search('customer_search_flight.html')


@app.route('/customer_search_flight_rt', methods=['GET', 'POST'])
def customer_search_flight_rt():
    return flight_search_rt('customer_search_flight.html')
...
def purchase():
    ...
    base_price = data['base_price'] = float(request.form['base_price'])
...
def make_purchase():
    ...
    cursor.close()
    return render_template('customer_search_flight.html', error=error, available=True)
...
def cancel_flight():
    ...
    connection.commit()
    cursor.close()
    return render_template('customer_my_flights.html', message='Flight Canceled')
...
def flight_search(page):
    ...
    return render_template(page, data=data, today_date=valid_date, available=available)


def flight_search_rt(page):
    # valid time/date
    timestamp = datetime.now()
    valid_timestamp = timestamp + timedelta(hours=2)
    valid_time = valid_timestamp.time()
